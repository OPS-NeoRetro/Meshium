package me.cortex.meshium;

import com.mojang.blaze3d.platform.GlStateManager;
import com.mojang.blaze3d.systems.RenderSystem;
import it.unimi.dsi.fastutil.ints.*;
import me.cortex.meshium.api0.MeshiumAPI;
import me.cortex.meshium.config.StatisticsLoggingLevel;
import me.cortex.meshium.config.TranslucencySortingLevel;
import me.cortex.meshium.gl.RenderDevice;
import me.cortex.meshium.gl.buffers.IDeviceMappedBuffer;
import me.cortex.meshium.managers.RegionManager;
import me.cortex.meshium.managers.RegionVisibilityTracker;
import me.cortex.meshium.managers.SectionManager;
import me.cortex.meshium.renderers.*;
import me.cortex.meshium.util.DownloadTaskStream;
import me.cortex.meshium.util.TickableManager;
import me.cortex.meshium.util.UploadingBufferStream;
...
import static me.cortex.meshium.gl.buffers.PersistentSparseAddressableBuffer.alignUp;
...
public class RenderPipeline {
    ...
    public RenderPipeline(RenderDevice device, UploadingBufferStream uploadStream, DownloadTaskStream downloadStream, SectionManager sectionManager) {
        ...
        this.compiledForFog = Meshium.config.render_fog;
    ...
    public void renderFrame(Viewport frustum, ChunkRenderMatrices crm, double px, double py, double pz) {//NOTE: can use any of the command list rendering commands to basicly draw X indirects using the same shader, thus allowing for terrain to be rendered very efficently
        ...
        for (int i = 0; i <3*3*3;i++) {
            new MeshiumAPI("meshium").setRegionTransformId(1, i%3, (i/3)%3, ((i/3)/3)%3);
        }
        new MeshiumAPI("meshium").setTransformation(1, new Matrix4f().identity().scale(1,1  ,1));
        new MeshiumAPI("meshium").setOrigin(1, 0,0,0);
           */
        ...
        //Enqueue all the visible regions
        {
            ...
            for (int i = 0; i < rm.maxRegionIndex(); i++) {
                ...
                if ((Meshium.config.region_keep_distance != 256 && Meshium.config.region_keep_distance != 32) && !rm.withinSquare(Meshium.config.region_keep_distance+4, i, chunkPos.x, chunkPos.y, chunkPos.z)) {
                    removeRegion(i);
                    continue;
                }
                ...
                if (rm.isRegionVisible(frustum, i)) {
                    ...
                    if (regionVisibilityTracker.get(i)) {//Going from visible to non visible
                        ...
                        if (Meshium.config.enable_temporal_coherence) {
            ...
            if (Meshium.config.statistics_level != StatisticsLoggingLevel.NONE) {
        ...
        if (Meshium.config.translucency_sorting_level == TranslucencySortingLevel.NONE) {
            regionsToSort.clear();
        }
    ...
    public void renderTranslucent() {
        ...
        //Download statistics
        if (Meshium.config.statistics_level.ordinal() > StatisticsLoggingLevel.FRUSTUM.ordinal()){
    ...
    public void addDebugInfo(List<String> info) {
        if (Meshium.config.statistics_level != StatisticsLoggingLevel.NONE) {
            ...
            if (Meshium.config.statistics_level.ordinal() >=  StatisticsLoggingLevel.REGIONS.ordinal()) {
                builder.append(", R: ").append(stats.regionCount);
            }
            if (Meshium.config.statistics_level.ordinal() >=  StatisticsLoggingLevel.SECTIONS.ordinal()) {
                builder.append(", S: ").append(stats.sectionCount);
            }
            if (Meshium.config.statistics_level.ordinal() >=  StatisticsLoggingLevel.QUADS.ordinal()) {
