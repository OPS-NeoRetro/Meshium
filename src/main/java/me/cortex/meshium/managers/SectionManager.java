package me.cortex.meshium.managers;
...
import me.cortex.meshium.sodiumCompat.IRepackagedResult;
import me.cortex.meshium.util.BufferArena;
...
public class SectionManager {
    ...
    public SectionManager(RenderDevice device, long fallbackMemorySize, UploadingBufferStream uploadStream, int quadVertexSize, MeshiumWorldRenderer worldRenderer) {
    ...
    public void uploadChunkBuildResult(ChunkBuildOutput result) {
        ...
        int terrainAddress;
        {
            ...
            if (terrainAddress == SegmentedManager.SIZE_LIMIT) {
                Meshium.LOGGER.error("Terrain arena critically out of memory, expect issues with chunks!! " +
                        " quad_used: " + this.terrainAreana.getUsedMB() +
                        " physically used: " + this.terrainAreana.getMemoryUsed() +
                        " limit: " + ((IMeshiumWorldRendererGetter)(SodiumWorldRenderer.instance())).getRenderer().getMaxGeometryMemory());
