...
import me.cortex.meshium.sodiumCompat.*;
...
public class MixinRenderSectionManager implements IMeshiumWorldRendererGetter {
    ...
    @Unique private MeshiumWorldRenderer renderer;
    @Unique private Viewport viewport;

    @Unique
    private static void updateMeshiumIsEnabled() {
        Meshium.IS_ENABLED = (!Meshium.FORCE_DISABLE) && Meshium.IS_COMPATIBLE && IrisCheck.checkIrisShouldDisable();
    }

    @Inject(method = "<init>", at = @At("TAIL"))
    private void init(ClientWorld world, int renderDistance, CommandList commandList, CallbackInfo ci) {
        updateMeshiumIsEnabled();
        if (Meshium.IS_ENABLED) {
            if (renderer != null)
                throw new IllegalStateException("Cannot have multiple world renderers");
            renderer = new MeshiumWorldRenderer(Meshium.config.async_bfs?new AsyncOcclusionTracker(renderDistance, sectionByPosition, world, rebuildLists):null);
            ((IMeshiumWorldRendererSetter)regions).setWorldRenderer(renderer);
        }
    }
    ...
    private ChunkVertexType modifyVertexType(ChunkVertexType vertexType) {
        updateMeshiumIsEnabled();
        if (Meshium.IS_ENABLED) {
            return MeshiumCompactChunkVertex.INSTANCE;
        }
        return vertexType;
    }
    ...
    private void destroy(CallbackInfo ci) {
        if (Meshium.IS_ENABLED) {
            ...
            ((IMeshiumWorldRendererSetter)regions).setWorldRenderer(null);
            renderer.delete();
            renderer = null;
        }
    }
    ...
    private void deleteSection(RenderSection section) {
        if (Meshium.IS_ENABLED) {
            if (Meshium.config.region_keep_distance == 32) {
    ...
    @Override
    public MeshiumWorldRenderer getRenderer() {
        return renderer;
    }
    ...
    private void redirectAnimatedSpriteUpdates(CallbackInfo ci) {
        if (Meshium.IS_ENABLED && Meshium.config.async_bfs && SodiumClientMod.options().performance.animateOnlyVisibleTextures) {
            ci.cancel();
